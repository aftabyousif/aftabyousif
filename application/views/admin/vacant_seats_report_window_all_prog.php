	...
	<div class="sparkline10-list">
		...
		<div class="sparkline10-hd">
			<div class="main-sparkline10-hd text-center bg-warning">
				<h1>Vacant Seats For All Degree Program Reporting</h1>
			</div>
		</div>
		...
		<div class="button-style-two btn-mg-b-10">
			<button type="button"  class="btn btn-custon-rounded-two btn-primary" ng-click="PrintDocument(fetchedVacantSeats)"><i class="fa fa-print edu-informatio" aria-hidden="true"></i> Print / Download Report</button>
			...
			<div class="asset-inner">
				<table class="table table-hover table-condesed">
					...
					<tr ng-repeat="z in fetchedVacantSeats  track by $index" style="font-size: 10pt">
...
<script type="text/javascript">
	...
	app.controller('formCtrl', function($scope,$http,$window) {
		...
		$scope.getVacantSeats	= function (program_type_id,shift_id,campus_id,program_id,session_id){
			...
			// $scope.programs = null;
			$http.post('<?=base_url()?>Selection_list_report/vacantSeatsDataAllProg',data).then(function success(response){
				if (response.status == 204 ){
					$scope.errorMsg= 'Sorry no data found';
				}
				if (response.status == 200 ){
					$scope.errorMsg=null;
					let array_data = response.data;
					$scope.fetchedVacantSeats = array_data[campus_id];
					/*
		...
		$scope.PrintDocument	= function (programSeats){
			var $popup = $window.open("printVacantAllProgSeatReport", "popup", "width=700,height=500,left=20,top=150");
			$popup.Val = {"data":programSeats};
