        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                ...
                <div class="product-status-wrap">
                    ...
                    <br/><br/>
                    <hr/>
                    <button type="button" id="download_unverified_list" class=" col-md-4 form-control  basic-ele-mg-b-10 responsive-mg-b-10 btn btn-sm btn-warning" title="click here to download unverfied challan list" style="background-color: #7991E8"><i class="fa fa-database"></i> <strong>Download Unverfied Challan List</strong></button>
                    <br/>
...
<script type="text/javascript">
    ...
    function import_db(){
        ...
        $.ajax({
            url:'<?=base_url()?>Cmd/import_challan_ledger_handler',
            type: 'POST',
            data: {list_of_verified_challan:JSON.stringify(list_of_verified_challans)},
            // dataType: 'json',
            success: function (data) {
                console.log(data);
                $('#msg').html("<p class='text-center'>"+data.MESSAGE+"</p>");
            },
            error: function (data) {
                console.log(data);
                var value = data.responseJSON;
                $('#msg').html("<p class='text-center'>"+value.MESSAGE+"</p>");
            }
        });
    }
            ...
            success: function (data) {
               ...
               // console.log(data)
                ...
                $.each(data,function (k,v){
                    ...
                    //console.log(DEPOSIT_SLIP_NO.startsWith("2000"));
                    let clr ="";
                    let challan_no;
                    if(DEPOSIT_SLIP_NO.startsWith("2000")){
                        challan_no = DEPOSIT_SLIP_NO.substring(4);
                        clr ="";
                    }
                    else if(DEPOSIT_SLIP_NO.startsWith("200")){
                        challan_no = DEPOSIT_SLIP_NO.substring(3);
                        clr ="pink";
                    }else{
                        clr ="red";
                        challan_no = 0;
                    }
                    // if("200042789"==DEPOSIT_SLIP_NO){
                    //     console.log(challan_no);
                    //     console.log(list_of_unverified[challan_no]);
                    //     console.log(challan_no in list_of_unverified);
                    //     console.log(v);
                    // }
                    if(challan_no in list_of_unverified){

                       // console.log(challan_no);
                        let challan_obj =list_of_unverified[challan_no];

                        if(challan_obj.CHALLAN_AMOUNT ==AMOUNT && challan_obj.FORM_CHALLAN_ID ==challan_no  ){
                            clr ="green";
                            list_of_verified_challans.push(challan_obj);
                            delete list_of_unverified[challan_no];

                        }else{
                            clr ="orange";
                            list_of_reject_list.push(v);
                        }
                    ...
                    row+="<tr style='background-color: "+clr+";'>";
                    row+="<td>"+sno+"</td>";
                    row+="<td>"+BRANCH_CODE+"</td>";
                    row+="<td>"+BRANCH_NAME+"</td>";
                    row+="<td>"+DEPOSIT_SLIP_NO+"</td>";
    ...
    $("#download_unverified_list").click(function(){
        var csvString = "";
        for (var i = 0; i < list_of_reject_list.length; i++) {
                var line = '';
                var heading = '';
                for (var index in list_of_reject_list[i]) {
                    if (line != '') line += ','
                    if (heading != '') heading += ','
                    
                     heading += '"'+index+'"';
                    line += '"'+list_of_reject_list[i][index]+'"';
                }
    ...
    var list_of_unverified = <?=$json_data?>;
    
    var list_of_verified_challans = [];
    
    var list_of_reject_list =[];
</script>
</body>
</html>
