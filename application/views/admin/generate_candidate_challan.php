		...
		app.controller('formCtrl', ['$scope','$window','$http', function($scope,$window,$http) {
			$scope.search = function(generateby) {
				...
				if (generateby == 'generatebyapplication') {
					...
					if (!$scope.prog_id) { $scope.errorMSG = "Program is required"; return; } else
					if (!$scope.validUpto) { $scope.errorMSG = "Valid upto is required"; return; } else
					...
					if (!$scope.challan_type_id) { $scope.errorMSG = "Challan type"; return; }

					let checkboxCalculatePaidFee=false;
					if ($scope.checkboxCalculatePaidFee){
						checkboxCalculatePaidFee=true;
					}
					let checkboxDueFee=false;
					if ($scope.checkboxDueFee){
						checkboxDueFee=true;
					}
					let checkboxEnrolmentFee=false;
					if ($scope.checkboxEnrolmentFee){
						checkboxEnrolmentFee=true;
					}
					...
					let Indata = {
						...
						prog_id:$scope.prog_id,
						semester_id:$scope.semester_id,
						fee_demerit_id:$scope.fee_demerit_id,
						valid_upto:$scope.validUpto,
						session_id:$scope.session_id,
						challan_type_id:$scope.challan_type_id,
						otherAmount:$scope.otherAmount,
						checkboxCalculatePaidFee:checkboxCalculatePaidFee,
						checkboxDueFee:checkboxDueFee,
						checkboxEnrolmentFee:checkboxEnrolmentFee
					};
					$scope.getChallanData(Indata);
			...
			$scope.getChallanData = function(Indata) {
				...
				$http.post('<?=base_url()?>FeesReports/getChallanData',Indata).then(function success(response){
			...
			$scope.generate_challan = function() {
					if ($window.confirm("Do you want to generate challan?") === false){
						return;
					}
				$scope.GenerateMSG="Processing your request please wait.";
				$http.post('<?=base_url()?>FeesReports/generate_candidate_challan',$scope.challan_info).then(function success(response){
					// console.log(response.data)
		...
		<div class="product-status-wrap">
			<div class="row">
				<div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
					<h4 style="font-size: 11pt">Generate Candidate Challan</h4>
				</div>
				<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 daidh">
					<label>Generate By</label>
					<select class="form-control" name="generateby" id="generateby" ng-model="generateby" ng-init="generateby = 'generatebyprogram'">
						<option value="generatebyprogram">Program</option>
						<option value="generatebyselectionlist">Selection List</option>
					</select>
				</div>
				<div class="col-lg-1 col-md-2 col-sm-2 col-xs-12 daidh" >
				...
				<div class="col-lg-1 col-md-1 col-sm-1 col-xs-12 daidh">
					<label>Part</label>
					<select class="form-control" ng-model="part_id" ng-change="getProgramTypes()">
				...
				<div class="col-lg-1 col-md-1 col-sm-4 col-xs-12">
					...
					<input type="text" class="form-control"  id="end_date" ng-model="validUpto" value="<?=date('Y-m-d')?>" title="yyyy-mm-dd" style="font-size: 7pt; font-weight: bold" />
				</div>
				<div class="col-lg-1 col-md-1 col-sm-4 col-xs-12 daidh">
					<label>Starting Challan #</label>
					<input type="text" class="form-control" ng-model="starting_challan_no"/>
				</div>
			...
			<div class="row">
				...
				<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 daidh">
					...
					<input type="text" class="form-control" ng-model="otherAmount"/>
				</div>
				<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 daidh">
					<ul>
						<li>
							<input type="checkbox" ng-model="checkboxCalculatePaidFee"/>
							<label>Calculate Paid Fee</label>
						</li>
						<li>
							<input type="checkbox" ng-model="checkboxEnrolmentFee"/>
							<label>Add Enrolment Fee</label>
						</li>
						<li>
							<input type="checkbox" ng-model="checkboxDueFee"/>
							<label>Calculate Due Fee</label>
						</li>
					</ul>
				</div>
				<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 daidh ">
			...
			<div id="generatebyprogram" ng-show="generateby=='generatebyprogram' || generateby=='generatebyselectionlist'">
				<div class="row">
					...
					<div class="col-lg-3 col-md-4 col-sm-12 col-xs-12" ng-show="generateby=='generatebyprogram'">
						<label>Program</label>
						<select class="form-control" ng-model="prog_id">
					    ...
					    <table class="table table-condesed">
							<thead>
    							<tr style="font-size: 9pt;">
									...
									<th class="bg-primary">CHALLAN AMT</th>
    								<th class="bg-danger">DUES</th>
    								<th class="bg-secondary">LATE FEE</th>
    								<th class="bg-warning">PAYABLE AMT</th>
							...
							<tbody>
    							<tr ng-repeat="challan in challan_info track by $index" style="font-size: 9pt;">
    								<td>{{$index + 1}}</td>								
    								<td>{{challan.CHALLAN_NO}}</td>
    								<td>{{challan.SELECTION_LIST_ID}}</td>
    								<td>{{challan.APPLICATION_ID}}</td>
    								<td>{{challan.ROLL_NO}}</td>
    								<td>{{challan.FIRST_NAME}}</td>
    								<td>{{challan.FNAME}}</td>
    								<td>{{challan.LAST_NAME}}</td>
    								<td>{{challan.PROGRAM_TITLE}}</td>
    								<td>{{challan.CATEGORY_NAME}}</td>
    								<td>{{challan.REMARKS}}</td>
    								
    								<td class="bg-primary">{{challan.CHALLAN_AMOUNT}}</td>
    								<td class="bg-danger">{{challan.DUES}}</td>
    								<td class="bg-secondary">{{challan.LATE_FEE}}</td>
    								<td class="bg-warning">{{challan.PAYABLE_AMOUNT}}</td>
    								<td class="bg-success">{{challan.PAID_AMOUNT}}</td>
            ...
            <br/><br/>
            <hr/>
            <button type="button" ng-click="generate_challan()" class=" col-md-4 form-control  basic-ele-mg-b-10 responsive-mg-b-10 btn btn-sm btn-warning" title="click here to generate fee challan to database" style="background-color: #7991E8"><i class="fa fa-database"></i> <strong>Generate Fee Challan</strong></button>
            <br/>
