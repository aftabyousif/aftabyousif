		...
		<div class='row'>
			<div class='col-md-6'>
				...
				<div class="col-md-4">
					...
					<div class="col-md-8">
					<label>Campus</label>
					<select ng-model="CampusModel" class="form-control" ng-change="getPrograms(ProgramTypesModel,ShiftsModel,CampusModel,SessionModel); getVacantSeats(ProgramTypesModel,ShiftsModel,CampusModel,ProgramModel,SessionModel,ListNoModel)">
						<option ng-repeat="campus in campuses" ng-value="{{campus.CAMPUS_ID}}">{{campus.NAME}}</option>
					</select>
				</div>
					<div class="col-md-4">
					<label>List No</label>
					<select ng-model="ListNoModel" class="form-control" ng-change=" getVacantSeats(ProgramTypesModel,ShiftsModel,CampusModel,ProgramModel,SessionModel,ListNoModel)">
						<option ng-repeat="l_n in list_no" ng-value="{{l_n.ADMISSION_LIST_ID}}">{{l_n.LIST_NO}}</option>
					</select>
				</div>
			...
			<div class='col-md-6'>
				...
				<select  ng-model="ProgramModel" ng-change="showVacantSeats(ProgramTypesModel,ShiftsModel,CampusModel,ProgramModel,SessionModel)" class="form-control" style='height:200px' multiple>
			...
			<div class="asset-inner">
				<table class="table table-hover table-condesed">
					...
					<tr style="font-size: 10pt;text-align: center">
						...
						<th>Fill Seats</th>
						<th>Vacant Seat</th>
						<th>Current Selection Fill Seat</th>
						<th>Total Vacant Seat With Selection</th>
						<th>Total Fill Seat With Selection</th>
					</thead>
					<tbody ng-repeat="x in DisplayCategories  track by $index">
					<tr style="font-size: 10pt;text-align: center">
						<th colspan="7" class="bg-info" style="text-align: center">{{x.CATEGORY_TYPE_NAME}}</th>
					</tr>
					<tr ng-repeat="z in x.CATEGORIES  track by $index" style="font-size: 10pt">
						...
						<td>{{z.CURRENT_FILLED_SEAT}}</td>
						<td>{{z.TOTAL_VACANT_SEAT_WITH_SELECTION}}</td>
						<td>{{z.TOTAL_FILLED_SEAT_WITH_SELECTION}}</td>
						<td>{{z.PROMOTED_SEAT}}</td>
					</tr>
					</tbody>
				</table>
			</div>
...
<script type="text/javascript">
	...
	app.controller('formCtrl', function($scope,$http,$window) {
		...
		$scope.getPrograms 		= function (program_type_id,shift_id,campus_id,session_id){
		...
		$scope.getVacantSeats	= function (program_type_id,shift_id,campus_id,program_id,session_id,admission_list_id){
			...
			else if (admission_list_id == null){
				$scope.errorMsg = "Select list no";
				return;
			}

			$scope.errorMsg = "Getting Records....";

			let data = {program_type_id:program_type_id,shift_id:shift_id,campus_id:campus_id,session_id:session_id,admission_list_id:admission_list_id};
			// $scope.programs = null;
			$http.post('<?=base_url()?>Selection_list_report/vacantSeatsDataWithCurrentSelection',data).then(function success(response){
				if (response.status == 204 ){
					$scope.errorMsg= 'Sorry no data found';
				}
				if (response.status == 200 ){
			...
			$scope.errorMsg = "Getting Records....";
					...
					angular.forEach(ProgramCategory,function (categoryTypes){
						angular.forEach(categoryTypes['CATEGORIES'],function (categories){
							...
							$scope.FilledSeats+=parseInt(categories['FILLED_SEATS']);
							$scope.VacantSeats+=parseInt(categories['VACANT_SEATS']);
						//	$scope.CurrentFilledSeat+=parseInt(categories['CURRENT_FILLED_SEAT']);
							console.log(categories);
						});
					});
		}
