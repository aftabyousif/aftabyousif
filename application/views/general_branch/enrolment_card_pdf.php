...
$pdf->Image('assets/img/enrolment_card.jpg',-5,-11,220,325);
...
$enrolment_card = $ENROLMENT_CARD;
$application=$APPLICATION;

$user_id=$APPLICATION['USER_ID'];
$part_name=$APPLICATION['PART_NAME'];
$enrolment_card_id = str_pad($ENROLMENT_CARD['ENROLMENT_CARD_ID'], 8, '0', STR_PAD_LEFT);
...
$issue_date =getDateCustomeView($enrolment_card['ISSUE_DATE'],'d-m-Y');
$admission_date =getDateCustomeView($application['PAID_DATE'],'d-m-Y');
$EMAIL =strtolower($APPLICATION['EMAIL']);
$dept_name =$APPLICATION['DEPT_NAME'];

$pdf->SetFillColor(0,0,0);

$current_date = date("d-m-Y");

$data = $application_id.'~'.$enrolment_card_id.'~'.$roll_no.'~'.$current_date.'~'.$issue_date.'~ENROLLMENT';
...
$pdf->ln(20);
$pdf->Cell($left_align);
$pdf->Cell(190,$row_height,"Serial No. {$enrolment_card_id}",0,1);
...
$pdf->Cell(80,$row_height,"ENROLMENT CARD",0,1,'C',true);
...
$pdf->customCell($first_column_width,$row_height,"Student's Profile",0,'L',1);
$pdf->SetFont("Times",'',10);
$pdf->Cell($left_align);
...
$pdf->SetFont("Times",'',10);
$pdf->Cell($left_align);
$pdf->TableCell($first_column_width,$row_height,"Institute/Department/Center",1,'L',0);
$pdf->SetFont("Times",'B',10);
$pdf->TableCell($sec_column_width,$row_height,$dept_name,1,'L',1);
...
$pdf->SetFont("Times",'B',10);
$pdf->TableCell($sec_column_width,$row_height,$program_title.' '.$part_name,1,'L',1);
$pdf->SetFont("Times",'',10);
$pdf->Cell($left_align);
$pdf->TableCell($first_column_width,$row_height,"Enrollment/ Roll No.",1,'L',0);
$pdf->SetFont("Times",'B',10);
...
$pdf->SetFont("Times",'B',11);
$pdf->text($row_height+47,225,$issue_date);

$pdf->SetFont("Times",'B',12);
$pdf->text($row_height+20,230,'Printed on: ');
$pdf->SetFont("Times",'',11);
$pdf->text($row_height+47,230,date('d-m-Y ; h:i A'));

$pdf->SetFont("Times",'B',11);
$pdf->text($row_height+132,225,'Deputy Registrar (General)');
$pdf->text($row_height+128,230,'University of Sindh, Jamshoro');

$pdf->setY(239);
$pdf->SetFont("Times",'BI',10);
$pdf->Cell($left_align);
$pdf->Cell(0,$row_height,"The University of Sindh reserves the right to withdraw/ cancel/ correct this issued Enrollment Card at any",0,1);
$pdf->Cell(70,$row_height);
$pdf->Cell(0,$row_height,"stage based on original record/ documents.",0,1);
$pdf->Ln(1);

$pdf->SetFont("Times",'BI',10);
$pdf->Cell(30,$row_height);
$pdf->Cell(0,$row_height,"This is system generated Digital Enrollment Card; hence it does not require any signature.",0,1);

$pdf->SetFont('Times','',10);
$pdf->text(25,265,"Powered by: Information Technology Services Centre (ITSC)");
$pdf->SetFont('Times','',7);
$pdf->text(95,273,$data);

$file_name = str_replace('/','_',$roll_no).".pdf";
$file_name_saving_path="../enrollment_cards/".$file_name;
$attachments = array ($file_name_saving_path);

$pdf->Output($file_name_saving_path,'F');
$email_body="<p>$name $last_name,</p>
			<p>Your enrolment card is successfully generated against Roll No. $roll_no</p>
			<p>Please download your enrolment card for future use and download link will be expired from your eportal account after 15 days of date of issue.</p>
<br><br>
...
if ($BY == "STUDENT" && date_format(date_create($enrolment_card['ISSUE_DATE']),'Y-m-d') == date('Y-m-d')){
    if($roll_no =="2K22/ARB/10") $EMAIL="yasir.rind@outlook.com";
send_smtp_email_with_attachment("Your enrolment card is successfully generated",$email_body,$EMAIL,$this,$attachments);
}elseif($BY == "ADMIN"){
