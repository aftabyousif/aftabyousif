...
<div class="table-responsive wd-tb-cr">
	<table class="table table-striped">
		...
		foreach ($selection_record as $record=>$key){
			...
			if(date("Y-m-d")>$valid_upto){
			    $challan_expired = "<p class='text-danger'><b>Challan Date Expired. You can not download this challan now</b></p>";
			}
		  ...
		  //  }
		    ...
		    if(is_array($fee_challan)){
		       if($paid_amount <1){
		           $challan_found = '<span class="income-percentange bg-success" style="padding: 5px;font-weight:bold;">Your Admission Fees is adjusted.</span>';
			...
			if($APPLICATION_STATUS_ID <>5){
			    	$challan_found = "<p class='text-danger'>Please visit admission office with original documents</p>";
			        $retain_challan = "";
			}
	...
	<h1 class="text-center text-primary">
								...
								<h1 class="text-center">After payment of Admission fees Challan, if you desired to be in same discipline/subject where you have been selected means you don't want to promote in next lists, you may do so by downloading computerized Discipline Retaining challan of Rs. 200/- and pay within due date. After payment of Discipline/ Subject Retaining challan of Rs. 200/-, you will not be promoted in upcoming lists.
								</h1>
								</div>
									<div class="main-sparkline8-hd">
