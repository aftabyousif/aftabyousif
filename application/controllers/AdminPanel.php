...
//		$this->load->library('javascript');
    ...
    public function test_sms(){
      $data = $this->Administration->get_enrolled_candidate_category_wise();
      $header = $this->Administration->get_all_cateorgy();
      $data = merge_list_with_key($data,'PROGRAM_TITLE');
     // prePrint($data);
      echo "<table border='1'>";
        echo "<tr>";
       echo "<td>$k</td>";
      ...
      foreach($data as $k=>$value){
       ...
       echo "<td>$k</td>";
           ...
           foreach($header as $k=>$v){
            $t = true;
            foreach($value as $d){
                if($v['CATEGORY_NAME']==$d['CATEGORY_NAME']){
                        echo "<td>{$d['TOTAL']}</td>";
                        $t = false;
                 }
            }
    ...
    public function get_basic_information(){
        ...
        if(isset($_POST['CNIC_NO']) && isValidData($_POST['CNIC_NO']) && isset($_POST['SEARCH_BY']) && isValidData($_POST['SEARCH_BY'])){
            ...
            if($SEARCH_BY == "APPLICATION_ID")
            {
                ...
                // $user_registration_data = $form_data['users_reg'];
               
                // if($user_registration_data['IS_CNIC_PASS']=="P"){
                //      $PASSPORT_NO = $user_registration_data['PASSPORT_NO'];
                // }else{
                //   $CNIC_NO = $user_registration_data['CNIC_NO'];  
                // }
                // print_r($user_registration_data);
                // exit();
        ...
        if($this->session->has_userdata('STUDENT_USER_ID')&&$this->session->userdata('STUDENT_USER_ID')) {
            ...
            $form_array = array("PASSWORD"=>$password);
             $res = $this->User_model->updateUserById($USER_ID, $form_array, $admin['USER_ID']);
                ...
                if ($res === 0 ) {
                   sendPasswordByEmail($user['EMAIL'],$token);
...
//        }
    ...
    public function student_application_unlock(){
        ...
        if(isset($_POST['USER_PASSWORD'])&&isset($_POST['IS_UNLOCK'])&&isValidData($_POST['USER_PASSWORD'])){
            $user = $this->session->userdata($this->SessionName);
            if($user['PROFILE']['PASSWORD']==cryptPassowrd(isValidData($_POST['USER_PASSWORD']))){
                if($this->session->has_userdata('STUDENT_USER_ID')&&$this->session->has_userdata('STUDENT_APPLICATION_ID')){
                                    ...
                                    echo "<td>".$metric['ROLL_NO']."</td>";
                                     ...
                                     echo "<td>".$qualification['OBTAINED_MARKS']."</td>";
                                      echo "<td>".$qualification['TOTAL_MARKS']."</td>";
                                       ...
                                       echo "<td><img width='100' height='100' src='".itsc_url().EXTRA_IMAGE_PATH.$qualification['MARKSHEET_IMAGE']."'/></td>";
                                      echo "<td><img width='100' height='100' src='".itsc_url().EXTRA_IMAGE_PATH.$qualification['PASSCERTIFICATE_IMAGE']."'/></td>";
...
//        exit();
	...
	public function get_all_candidate_challan_verification(){
       ...
       $BRANCH_ID =$offset = $IS_PROFILE_PHOTO_VERIFIED = $DISTRICT_ID= $challan_upload = $limit=$is_verified=$gender=$date_time=null;
    ...
    //   $photodata = $this->AdmitCard_model->getDataForPhoto($date_time,$program_type_id,$gender,$is_verified,$limit,$challan_upload,$DISTRICT_ID,$IS_PROFILE_PHOTO_VERIFIED,$PROVINCE_ID);
           $list  =  $this->FeeChallan_model->get_form_challan_for_verification($is_verified,$limit,$offset,$BRANCH_CODE);
         $user = $this->session->userdata($this->SessionName);
        $user_role = $this->session->userdata($this->user_role);
        $user_id = $user['USER_ID'];
        $role_id = $user_role['ROLE_ID'];
	...
	public function get_all_candidate_image(){
       ...
       $IS_PROFILE_PHOTO_VERIFIED = $DISTRICT_ID= $challan_upload = $limit=$is_verified=$gender=$date_time=null;
        if(isset($_GET['IS_VERIFIED'])){
            $is_verified = $_GET['IS_VERIFIED'];
        }
        if(isset($_GET['DATE_TIME'])){
    ...
    Dear Candidate,<br><br>Your uploaded challan image is not verified, that's why your Admit card will not be issued. If you have paid the challan within due dates, please visit Directorate of Admissions, University of Sindh, Jamshoro along with original paid copy of your challan upto Thursday 03.11.2022 within office hours (09:00am to 03:00pm) OR you can send it through email at admission@usindh.edu.pk
        ...
        if(isset($_GET['APPLICATION_ID'])&&isset($_GET['STATUS'])&&isValidData($_GET['APPLICATION_ID'])){
            ...
            if($status>=0 && $status <= 2){
                if($status == 1){
                    ...
                    $email_subject ='PROFILE PHOTO / IMAGE NOT VERIFIED';
    $email_body = "<img src='https://usindh.edu.pk/wp-content/uploads/2018/10/2logo-usindh.png'> <br> <p style='font-size:14pt'>السلام عليكم</p><br>
    Dear Candidate,<br><br>Your profile photo has been rejected due to inappropriate photo, kindly re-upload your profile photo otherwise your admit card will not be issued.
...
// 			exit();
            if ($application) {
                ...
                if($application['PROGRAM_TYPE_ID']==2){
                    ...
                    //                             	redirect(base_url('form/dashboard'));
                    //                                 exit();
                    $show_program = array("1"=>array(65,50,178,149,180,181,261),
                        "2"=>array(50),
                        "4"=>array(50),
                        "5"=>array(50));
...
//                    $data['form_data'] = $form_data;
    ...
    public function view_all_session_handler(){
        if(isset($_POST['add'])||isset($_POST['update'])){
            ...
            if($error==""){
                $form_array = array("YEAR"=>$year,"REMARKS"=>$remarks,"DATE"=>$date,"BATCH_REMARKS"=>$batch_remarks,"SESSION_CODE"=>$sessoin_code);
                if($session_id){
