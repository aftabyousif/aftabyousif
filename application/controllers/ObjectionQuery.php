...
class ObjectionQuery extends AdminLogin
{
	...
	{		
		$USER_ID=$_SESSION['USER_LOGIN_FOR_ADMISSION']['USER_ID'];
		$cnic=$_SESSION['USER_LOGIN_FOR_ADMISSION']['CNIC_NO'];
		$this->load->model('User_model');
		$data['data'] = $this->User_model->getUserByCnic($cnic);
		$data['userTickets'] = $this->Query_model->getUserTicketByCnic($USER_ID);
        ...
        $this->load->view("E_ticket/open_ticket",$data);
	...
	public function ticketSubmit ()
	{
		...
		$this->form_validation->set_rules('objection_type', 'objection_type', 'trim|required|min_length[2]');
		...
		if($this->form_validation->run())
		{
			...
			$contact_no=$data['data']['MOBILE_NO'];
			$ref=$data['data']['CNIC_NO'];
			
			$objection_type = html_escape(htmlspecialchars($this->input->post('objection_type')));
...
//		$this->email->bcc('director.itsc@usindh.edu.pk');
    ...
    <p>Support Team</p>
    ";
		$this->email->from('no-reply@usindh.edu.pk','Directorate of Admissions');
		$this->email->to($to);
...
//		$this->email->bcc('director.itsc@usindh.edu.pk');
    ...
    <a href='".base_url()."ViewAllQueries/ticketResponse/".$ticket_no."'>CLICK HERE to give response</a>
    <br/>
...
//			echo "OK";
	...
	public function upload_ticket_image($ticket_no)
	{
		...
		$config['upload_path']          = '../eportal_resource/ticket_uploads/';
